...
class Matriks():
    ...
    def identity_eigen(size=(3,3), fills=0):
        m = Matriks(size=size)
        m.fill(0)
    ...
    def flip(self):
        mTemp = self.copy()
        for i in range(mTemp.rows):
            mTemp.mat[i] = self.mat[-i-1]
        return mTemp

    def reduksi_determinan(m):
        # Matriks yang diterima adalah yang sudah disimplified
        mTemp = m.copy()
        colLead = 0
        rowLead = 0
    ...
    def norm_matrix(self):
        ...
        for i in range(self.rows):
            normRat = sum(map(lambda x:x*x, self.mat[i])) ** 0.5
            if normRat != 0:
                self.mat[i] = [j/normRat for j in self.mat[i]]
